...
class WP_Import_Extend extends WP_Importer {
	...
	function import_start( $file ) {
        ...
        foreach($import_data['posts'] as $post){
			if(in_array($post['post_title'],$this->exclude_post_title)) {	
				continue;
			}
...
<?php
	...
	function fetch_remote_file( $url, $post ) {
		...
		if ( $upload['error'] ){
			$this->error_msg[] = 'Upload error:' . $url .' ' . $upload['error'];
			return;
		}
		...
		if ( ! $headers ) {
			@unlink( $upload['file'] );
			$this->error_msg[] =  'File error: '. $url .' '.__('Remote server did not respond', 'wordpress-importer');
			return;
		}
		...
		if ( $headers['response'] != '200' ) {
			@unlink( $upload['file'] );
			$this->error_msg[] =  'File error: '. $url .' '.sprintf( __('Remote server returned error response %1$d %2$s', 'wordpress-importer'), esc_html($headers['response']), get_status_header_desc($headers['response']) ) ;
			return ;
		}
		...
		if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
			@unlink( $upload['file'] );
			$this->error_msg[] = 'File error: ' . $url .' '. __('Remote file is incorrect size', 'wordpress-importer') ;
			return;
		}

		if ( 0 == $filesize ) {
			@unlink( $upload['file'] );
			$this->error_msg[] = 'File error: ' . $url .' '. __('Zero size file downloaded', 'wordpress-importer') ;
			return;
		}
		...
		if ( ! empty( $max_size ) && $filesize > $max_size ) {
			@unlink( $upload['file'] );
			$this->error_msg[] = 'File error: ' . $url .' '.sprintf(__('Remote file is too large, limit is %s', 'wordpress-importer'), size_format($max_size) )  ;
			return;
		}
